...
const ProductManager = () => {
    ...
    const handleProduct = (e) => {
        // 리액트에서는 UI요소와 바인딩된 데이터 소스(객체-참조타입)의 값이 변경할 때
        // 데이터 소스가 변경되었는지를 자동으로 감지합니다.
        // 그런데 참조타입을 그냥 복사하거나 그냥 속성의 값을 그대로 변경하는경우 메모리주소가 변경되지 않기 때문에 변경을 감지할 수 없다.
        // 그래서 DeepCopy의 일종인 spread연산자를 통해 원본 데이터 소스의 복사본(DeepCopy)를 만들어
        // 복사본의 새로운 메모리주소와 공간을 할당해서 객체가 변경되게 처리하여 데이터 변경사실을 데이터바인딩 엔진에게 알려준다.

        // 하기방식대로 직접적인 속성값을 변경하거나 복사본을 만들어서 변경해도
        // 리액트 데이터바인딩 엔진에서는 참조타입의 데이터 변경사실을 인지하지 못해 화면요소를 재렌더링하지 않는다.
        // product.productName = '이보람'
        // var newProduct = product; // swallow copy
        // newProduct.productName = 'test'

        // 그래서 결론은 참조타입의 데이터를 사용해 리액트에서 데이터바인딩을 사용하려면
        // 참조타입의 속성/값이 변경시킬때는 반드시 DeepCopy(spread연산자)를 통해 별도 복사본을 만들고
