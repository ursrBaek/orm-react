...
// Spread 연산자는 배열/객체의 복사/병합시 주로 사용한다.
...
// 스프레드 연산자는 ...복사할배열변수 형식으로 복사본을 만든다(별도의 메모리 공간에 할당됨)
...
const user2 = { ...user }; // Deep Copy: 분리된 복사본을 복사하기/ 1Depth까지만 복사
user2.age = 30;
user2.address.sido = "서울특별시"; // DeepCopy는 Spread는 1Depth 속성까지만 딥카피가 되고 2Depth이상은 원본소스의 값을 공유한다.
...
// Spread 연산자를 이용해 복사본을 만들고 기존의 속성값을 변경하거나 신규속성을 추가하여 신규 객체를 생성할 수 있다.
const myCar = { ...car, brand: "Volvo", price: 5000 };

console.log("myCar", myCar);

// 두개의 별도 다른 객체도 쉽게 병합이 가능합니다.
const company = {
  companyName: "Volvo Auto",
  country: "Germany",
...
// 2Depth 이상의 DeepCopy 사용하기
// 2Depth 이상의 속성의 값이 배열인경우는 array.map() 메소드를 통해 배열안의 객체를 복사(DeepCopy)해서 DeepCopy된 배열복사본을 새로 만들어서 할당한다.
// 2Depth 이상의 속성의 값이 단일 객체인 경우는 오리지널소스의 복사본(DeepCopy=spread연산자를 통해)을 새로 만들고 속성의 값을 재할당한다.
const newOrder = {
  ...order,
  products: [
    // map((p)=>{} 에서 p에는 order에 products의 배열값이 담긴다.)
    ...order.products.map((p) => {
      //map을 통해서는 배열의 복사본(DeepCopy)만 만들어지고 배열내의 객체들은 추가로 DeepCopy를 진행해줘야합니다.
      return { ...p }; // 배열내의 객체들의 DeepCopy 처리하기
      // return p; // return p 하게 되면 배열 그대로 복사본을 생성해서 추가한다. (swallow copy 원본을 그대로 사용한다)
    }),
  ],
  address: { ...order.address },
};

// 2Depth이상 속성을 DeepCopy한 속성의 값을 바꾼후 원본값과 비교해보기
newOrder.amount = 50000;
