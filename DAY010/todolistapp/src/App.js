...
function App() {
    ...
    const [todos, setTodos] = useState([
        ...
        { id: 3, text: '프론트엔드 기술 습득하기', desc: 'html, css, react cra, nextjs', checked: false },
    ])

    // 할일고유번호 데이터 정의 및 초기값할당 = 원시타입정의
    // const [nextId, setNextId] = useState(4)
    // todos의 마지막 길이 +1 (번호채번)
    const [nextId, setNextId] = useState(todos.length + 1)

    // 할일등록 처리 이벤트 처리함수 정의
    // 처리해야할 데이터가 존재하는 컴포넌트에서 이벤트처리함수를 정의하고 자식요소로 props를 통해 전달할 수 있다.
    // 자식요소에서는 전달된 이벤트 함수를 실행시킬 수 있고해당함수는 결국 부모 컴포넌트의 데이터를 변경하게 된다.
    ...
    // 클릭한 특정 할일의 체크박스에 바인딩된 개별 데이터객체의 checked 속성의 값을 변경해준다.
    const onSelect = (id) => {
        // 할일목록안에 특정 단일 아이템의 값을 바꾸는것이기 때문에 setTodos()를 호출하고 호출시 특정아이템을 찾고 관련속성값을 변경한다.
        // todo목록데이터의 map()메소드를 호출해서 배열복사본을 만들고 배열복사본을 반복해서 동일한 id값이 있는 객체를 찾은후
        // 동일한 객체가 있는경우 해당객체의 복사본(DeepCopy)를 실시하고 복사본의 특정값고 변경해서 신규객체를 배열에 반환하고 그렇지 않으면
        // 기존복사본(shawllow copy)본을 제공한다.
    ...
    return (
        <div className="App">
            ...
            <TodoTemplate>
                {/* 부모에서 정의된 함수(이벤트핸들러함수)도 props 방식으로 자식요소에 전달이 가능하다. */}
