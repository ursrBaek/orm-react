...
// 스타일 파일을 직접 만들어서 외부에서 참고 가능하다.
import './App.css'

function App() {
  // JSX영역 요소에 대한 기능 로직(프로그래밍기능)을 구현하는 영역
  // 순수 자바스크립트로 기능 구현하는 영역

  const userName = '고민준1'
  let userRole = '개발자'

  // 해당 컴포넌트 내에서만 사용하는 공통 스타일 정의 -엠베디드 스타일 적용방식
  // JSX에 대한 임베디드 스타일은 객체 정의 방식으로 스타일 속성과 값을 정의합니다.
  const myStyle = {
    ...
    <>
      <div className="App">
        <header className="App-header">
          ...
          {/* JSX 요소에 직접 인라이트 스타일하려면 {{}} 구조안에 명시해야 한다. */}
          ...
          {userName === '고민준' ? <h4>{userName}님 반갑습니다.</h4> : <h4>게스트님 반갑습니다.</h4>}
